...
public class SampleServerStartup extends BaseServerStartup {
	...
	protected Map<Integer, ChannelInitializer> choosePortsAndChannels(ChannelGroup clientChannels,
		...
		/*
		 * The below settings can be used when running behind an ELB TCP
		 * listener with proxy protocol, terminating SSL in Zuul.
		 */
		case HTTP2:
			sslConfig = ServerSslConfig.withDefaultCiphers(loadFromResources("server.cert"),
					loadFromResources("server.key"), WWW_PROTOCOLS);
		 ...
		 * listener with proxy protocol, terminating SSL in Zuul.
		 *
		 * Can be tested using certs in resources directory: curl
		 * https://localhost:7001/test -vk --cert
		 * src/main/resources/ssl/client.cert:zuul123 --key
		 * src/main/resources/ssl/client.key
		 */
		case HTTP_MUTUAL_TLS:
			sslConfig = new ServerSslConfig(WWW_PROTOCOLS, ServerSslConfig.getDefaultCiphers(),
					loadFromResources("server.cert"), loadFromResources("server.key"), null, ClientAuth.REQUIRE,
					loadFromResources("truststore.jks"), loadFromResources("truststore.key"), false);
