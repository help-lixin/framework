...
public class ZuulSampleModule extends AbstractModule {
	...
	protected void configure() {
		...
		bind(SessionContextDecorator.class).to(ZuulSessionContextDecorator.class); // decorate
																					// new
																					// sessions
																					// when
																					// requests
																					// come
																					// in
		bind(Registry.class).to(DefaultRegistry.class); // atlas metrics
														// registry
		bind(RequestCompleteHandler.class).to(BasicRequestCompleteHandler.class); // metrics
																					// post-request
																					// completion
		bind(AbstractDiscoveryClientOptionalArgs.class).to(DiscoveryClient.DiscoveryClientOptionalArgs.class); // discovery
																												// client
		bind(RequestMetricsPublisher.class).to(BasicRequestMetricsPublisher.class); // timings
																					// publisher
