...
import help.lixin.framework.config.exception.ConfigurationException;
import help.lixin.framework.config.util.CollectionUtils;
import help.lixin.framework.config.util.ResourceUtils;
import help.lixin.framework.config.util.StringUtils;

public class Config implements Map<String, Config.Section> {

	public static final String DEFAULT_SECTION_NAME = ""; // empty string means
															// the first unnamed
															// section
	 ...
	 *
	 * @param defaults
	 *            the default sections and/or key-value pairs to copy into the
	 *            new instance.
	 */
	...
	/**
	 * Returns {@code true} if no sections have been configured, or if there are
	 * sections, but the sections themselves are all empty, {@code false}
	 * otherwise.
	 *
	 * @return {@code true} if no sections have been configured, or if there are
	 *         sections, but the sections themselves are all empty,
	...
	/**
	 * Returns the names of all sections managed by this {@code Ini} instance or
	 * an empty collection if there are no sections.
	 *
	 * @return the names of all sections managed by this {@code Ini} instance or
	 *         an empty collection if there are no sections.
	 */
	...
	/**
	 * Returns the sections managed by this {@code Ini} instance or an empty
	 * collection if there are no sections.
	 *
	 * @return the sections managed by this {@code Ini} instance or an empty
	 *         collection if there are no sections.
	 */
	...
	/**
	 * Returns the {@link Section} with the given name or {@code null} if no
	 * section with that name exists.
	 *
	 * @param sectionName
	 *            the name of the section to retrieve.
	 * @return the {@link Section} with the given name or {@code null} if no
	 *         section with that name exists.
	 */
	...
	/**
	 * Ensures a section with the specified name exists, adding a new one if it
	 * does not yet exist.
	 *
	 * @param sectionName
	 *            the name of the section to ensure existence
	 * @return the section created if it did not yet exist, or the existing
	 ...
	 * {@code null} if the section did not exist.
	 *
	 * @param sectionName
	 *            the name of the section to remove.
	 * @return the section with the specified name or {@code null} if the
	...
	private static String cleanName(String sectionName) {
		...
		if (name == null) {
			// TODO
			// log.trace("Specified name was null or empty. Defaulting to the
	...
	/**
	 * Sets a name/value pair for the section with the given {@code sectionName}
	 * . If the section does not yet exist, it will be created. If the
	 * {@code sectionName} is null or empty, the name/value pair will be placed
	 * in the default (unnamed, empty string) section.
	 *
	 * @param sectionName
	...
	/**
	 * Returns the value of the specified section property, or {@code null} if
	 * the section or property do not exist.
	 *
	 * @param sectionName
	 *            the name of the section to retrieve to acquire the property
	 *            value
	 * @param propertyName
	 *            the name of the section property for which to return the value
	 * @return the value of the specified section property, or {@code null} if
	 *         the section or property do not exist.
	 */
	 ...
	 *            the name of the property to add
	 * @param defaultValue
	 *            the default value to return if the section or property do not
	 *            exist.
	 * @return the value of the specified section property, or the
	 *         {@code defaultValue} if the section or property do not exist.
	 */
	...
	/**
	 * Creates a new {@code Ini} instance loaded with the INI-formatted data in
	 * the resource at the given path. The resource path may be any value
	 * interpretable by the {@link ResourceUtils#getInputStreamForPath(String)
	 * ResourceUtils.getInputStreamForPath} method.
	 *
	 * @param resourcePath
	 *            the resource location of the INI data to load when creating
	 *            the {@code Ini} instance.
	 * @return a new {@code Ini} instance loaded with the INI-formatted data in
	 *         the resource at the given path.
	 * @throws ConfigurationException
	 *             if the path cannot be loaded into an {@code Ini} instance.
	 */
	public static Config fromResourcePath(String resourcePath) throws ConfigurationException {
	...
	/**
	 * Loads data from the specified resource path into this current {@code Ini}
	 * instance. The resource path may be any value interpretable by the
	 * {@link ResourceUtils#getInputStreamForPath(String)
	 * ResourceUtils.getInputStreamForPath} method.
	 *
	 * @param resourcePath
	 *            the resource location of the INI data to load into this
	 *            instance.
	 * @throws ConfigurationException
	 *             if the path cannot be loaded
	 */
	...
	/**
	 * Loads the INI-formatted text backed by the given InputStream into this
	 * instance. This implementation will close the input stream after it has
	 * finished loading. It is expected that the stream's contents are UTF-8
	 * encoded.
	 *
	 * @param is
	 *            the {@code InputStream} from which to read the INI-formatted
	 *            text
	 * @throws ConfigurationException
	 *             if unable
	 */
	...
	/**
	 * Loads the INI-formatted text backed by the given Reader into this
	 * instance. This implementation will close the reader after it has finished
	 * loading.
	 *
	 * @param reader
	 *            the {@code Reader} from which to read the INI-formatted text
	 */
	public void load(Reader reader) {
		...
		try {
			...
			try {
				...
				// TODO lixin
				// log.debug("Unable to cleanly close the InputStream scanner.
				// Non-critical - ignoring.", e);
			}
		}
	}
	...
	/**
	 * Loads the INI-formatted text backed by the given Scanner. This
	 * implementation will close the scanner after it has finished loading.
	 *
	 * @param scanner
	 *            the {@code Scanner} from which to read the INI-formatted text
	 */
	public void load(Scanner scanner) {
		...
		while (scanner.hasNextLine()) {
			...
			if (newSectionName != null) {
				// found a new section - convert the currently buffered one into
				// a Section object
				...
				// log.debug("Parsing " + SECTION_PREFIX + sectionName +
				// SECTION_SUFFIX);
				// }
			} else {
	...
	public String toString() {
		if (CollectionUtils.isEmpty(this.sections)) {
			...
			int i = 0;
			for (Config.Section section : this.sections.values()) {
				if (i > 0) {
					sb.append(",");
				}
	...
	/**
	 * An {@code Ini.Section} is String-key-to-String-value Map, identifiable by
	 * a {@link #getName() name} unique within an {@link Ini} instance.
	 */
