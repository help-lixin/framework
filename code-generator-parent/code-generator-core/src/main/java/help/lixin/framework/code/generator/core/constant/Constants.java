package help.lixin.framework.code.generator.core.constant;

public class Constants {

    public static final String POM_TARGET_PROJECT = "pom.targetProject";
    public static final String POM_GROUP_ID = "pom.groupId";
    public static final String POM_ARTIFACTED_ID = "pom.artifactId";
    public static final String POM_VERSION = "pom.version";
    public static final String POM_XML_TEMPLATE_PATH = "pom.xml.templatePath";


    public static final String CONTROLLER_TARGET_PROJECT = "controller.targetProject";
    public static final String CONTROLLER_TARGET_PACKAGE = "controller.targetPackage";


    public static final String SERVICE_TARGET_PROJECT = "service.targetProject";
    public static final String SERVICE_TARGET_PACKAGE = "service.targetPackage";
}
